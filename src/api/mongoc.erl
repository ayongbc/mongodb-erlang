        ...
        {ok, Pool = #{pool := C}} ->
            try poolboy:transaction(C, fun(Worker) -> Transaction(Pool#{pool => Worker}) end, Timeout)
                ...
                error:not_master ->
                    ...
                    error_logger:warning_msg("transaction error:~p reason:~p~n", [error, not_master]),
                    {error, not_master};
                ...
                E:R ->
                    mc_topology:update_topology(Topology),
                    error_logger:warning_msg("transaction error:~p reason:~p~n", [E, R]),
                    {error, R}
            end;
        Error ->
            error_logger:warning_msg("mc_topology get_pool error:~p~n", [Error]),
            Error
    end.
    ...
    case mc_topology:get_pool(Topology, Options) of
        {ok, Pool = #{pool := C}} ->
            try poolboy:transaction(C, fun(Worker) -> Transaction(Pool#{pool => Worker}) end, Timeout)
                ...
                error:not_master ->
                    mc_topology:update_topology(Topology),
                    error_logger:warning_msg("transaction_query error:~p reason:~p~n", [error, not_master]),
                    {error, not_master};
                error:{bad_query, {not_master, _}} ->
                    mc_topology:update_topology(Topology),
                    error_logger:warning_msg("transaction_query error:~p reason:~p~n", [error, not_master]),
                ...
                E:R ->
                    mc_topology:update_topology(Topology),
                    error_logger:warning_msg("transaction_query error:~p reason:~p~n", [E, R]),
                    {error, R}
            end;
        Error ->
            error_logger:warning_msg("mc_topology get_pool error:~p~n", [Error]),
