...
read(Connection, Request) ->
    read(Connection, Request, undefined).
...
read(Connection, Request = #'op_msg_command'{database = DB, command_doc = ([{_, Collection} | _] = Fields)}, _CmdBatchSize) ->
...
reply(#op_msg_response{response_doc = (#{<<"cursor">> := #{<<"firstBatch">> := Batch, <<"id">> := Id}} = Doc)})
    when map_get(<<"ok">>, Doc) == 1 ->
    {Id, Batch};
