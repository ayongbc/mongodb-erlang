...
put_message(Db, #getmore{collection = Coll, batchsize = Batch, cursorid = Cid}, _RequestId) ->
    <<?put_header(?GetmoreOpcode),
        ...
        ?put_int32(Batch),
        ?put_int64(Cid)>>;
put_message(Db, #op_msg_write_op{database = undefined} = OpMsg, _RequestId) ->
    <<
        ?put_header(?OpMsgOpcode),
        ?put_uint32(0), % Flags
        (put_section_type_zero(OpMsg#op_msg_write_op{database = make_bin(Db)}))/binary
    >>;
put_message(_Db, #op_msg_write_op{} = OpMsg, _RequestId) ->
    <<
        ?put_header(?OpMsgOpcode),
        ?put_uint32(0), % Flags
        (put_section_type_zero(OpMsg))/binary
    >>;
put_message(Db, #op_msg_command{database = undefined} = OpMsg, _RequestId) ->
    <<
        ?put_header(?OpMsgOpcode),
        ?put_uint32(0), % Flags
        (put_section_type_zero(OpMsg#op_msg_command{database = make_bin(Db)}))/binary
    >>;
put_message(_Db, #op_msg_command{} = OpMsg, _RequestId) ->
    <<
        ?put_header(?OpMsgOpcode),
        ?put_uint32(0), % Flags
        (put_section_type_zero(OpMsg))/binary
...
put_section_type_zero(#op_msg_write_op{
    ...
    Msg = [
        {erlang:atom_to_binary(Command, utf8), Collection},
        {<<"$db">>, Database}
    ] ++
        ExtraFields
        ++
