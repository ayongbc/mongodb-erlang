...
%%    UseLegacyProtocol = mc_utils:get_value(use_legacy_protocol, TopologyOptions, true),
%%    application:set_env(mongodb, use_legacy_protocol, UseLegacyProtocol),
...
handle_server_to_unknown(Server, #topology_state{servers = Tab} = State) ->
    case ets:select(Tab, [{#mc_server{pid = Server, _ = '_'}, [], ['$_']}]) of
        [] -> %% mc_server 崩溃时可能先从ets删除了Server的记录在发转换状态的消息，需要做匹配兼容
            State;
        [Saved] ->
            ToUpdate = #mc_server{
...
parse_seeds({srv, Addr}) ->
    {ok, Seeds} = mc_utils:get_srv_seeds(Addr),
    Seeds1 = [H ++ ":" ++ integer_to_list(P) || {H, P} <- Seeds],
    {unknown, undefined, Seeds1};
