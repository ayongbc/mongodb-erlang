...
init_seeds([], _, _, _) -> ok;
init_seeds([{Host, Port} | Seeds], Tab, Topts, Wopts) ->
    Host1 = mc_utils:to_binary(Host ++ ":" ++ integer_to_list(Port)),
    Saved = ets:select(Tab, [{#mc_server{host = Host1, _ = '_'}, [], ['$_']}]),
    start_seed(Saved, Host1, Tab, Topts, Wopts),
    init_seeds(Seeds, Tab, Topts, Wopts);
